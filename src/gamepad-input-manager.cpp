...
GamepadInputManager::GamepadInputManager(float thumbDeadZoneIn, float triggerDeadZoneIn)
	: connectedControllerId(-1)
{
	ZeroMemory(&previousState, sizeof(XINPUT_STATE));
	ZeroMemory(&currentState, sizeof(XINPUT_STATE));
	setThumbDeadZone(thumbDeadZoneIn);
	setTriggerDeadZone(triggerDeadZoneIn);
}
