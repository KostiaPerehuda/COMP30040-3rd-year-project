...
LucasKanadeOpticalFlow::LucasKanadeOpticalFlow(std::string windowName) : windowName_(windowName), shouldFindCorners_(true) {
}
...
void LucasKanadeOpticalFlow::onImageReceived(cv::Mat image) {
	...
	if (shouldFindCorners_ || previousPoints_.size() <= 5) {
...
//	cv::calcOpticalFlowPyrLK(previousGrayImage_, grayImage, previousPoints_, newPoints, status, err, cv::Size(15,15), 2, criteria);
	...
	for(uint i = 0; i < previousPoints_.size(); i++)
	{
		...
		if(status[i] == 1) {
			...
			cv::circle(image, newPoints[i], 5, cv::Scalar(0, 100, 255), -1);
		}
	}

	cv::Mat img;
	cv::add(image, mask_, img);
